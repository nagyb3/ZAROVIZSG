...
Itt két paraméter van: s, t
Legjobb mód lesz a felületek megadásához, mivel képes magaalá görbülni és eléggé gyorsan számítható
...
naiv: azért "visszalépés" megoldás keresés, mert ha eljutunk a keresés közben egy olyan pontra ahonnan már tudjuk, hogy nem fogunk megoldást találni (pl nem tudunk már úgy egy változónak sem értéket adni, hogy kielégítsük a kényszereket), akkor a keresést leállítjuk és egy másik ágon folytatjuk tovább a keresést.
### Kényszer kielégítési problémák:
Olyan problémákat írnak, le ahol az állapotok változókat tartalmaz, ezekhez a változókhoz értékeket szeretnék rendelni és változó értékeire szeretnék bizonyos kényszereket alkalmazni.
...
A változók értékeire alkalmazunk valamilyen fajta kényszereket is, amelyeknek az állapotoknak meg kell felelnie.
...
Keresési stratégiák csoportosítása:
- teljesség: ha van megoldás, akkor megtalálja
- időbonyolultság: hány csúcsot generál/terjeszt ki
- tárbonyolultság: egyszerre mennyi csúcsot kell memóriában tartani
- optimalitás: mindig a legkisebb költségű megoldást találja meg

Konstansok:
- b: maximális elágazási faktor a kereső fában
- d: a legkisebb költségű megoldás mélysége
- m: állapottér maximális mélységee (ez lehet végtelen is)

Ezeknél a keresőknél a csúcs az nem csak állapotot tárol hanem a következőeket együtt:
- szülő
- gyermek
- mélység
- útiköltség
...
A szisztematikus keresések nem használnak heurisztikát, amellyel képesek lennének kiértékelni, hogy a csúcsok mennyire vannak közel a célállapothoz.
...
- Egyenletes költségű keresés: sor alapú, mindig a legkisebb költségű még nem kiterjesztett csúcsot terjeszti ki
...
A szisztematikus fakeresők összehasonlítása:
![[Pasted image 20250530215930.png]]
megj:
- szélességi akkor optimális, ha minden lépés azonos költségű, de az általában nem teljesül.
- Iteratívan mélyülő mélységi akkor optimális, ha az élköltség 1

#### Szisztematikus gráf kereső:

#### Heuriszitkus kereső:
- alapötlet: használjunk egy kiértékelő függvényt, amely a csúcsokra megmodja, hogy az adott csúcs milyen messze van a cél célállapottól (ez lesz a heurisztikus függvény)
...
NEM OPTIMÁLIS (miért?)
Időbonyolultság: O(b^m), de jó heurisztika felgyorsíthatja
Tárbonyolultság: O(b^m), minden csúcsot a memóriában

### A* keresés
alapötlet: ne terjesszük ki azokat az utakat, amelyek már alapból drágák, vagyis ne csak a csúcs céltól való távolságát nézzük, hanem azt is mennyire van messze a csúcs
...
- TELJES, ha a csúcs száma véges
- OPTIMÁLIS
- Időbonyolultság: exponenciális ??
